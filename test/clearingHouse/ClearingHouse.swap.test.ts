...
describe("ClearingHouse.swap", () => {
    ...
    describe("# swap", () => {
        beforeEach(async () => {
            ...
            expect(await vault.getFreeCollateral(alice.address)).to.eq(toWei(10, collateralDecimals))
            await clearingHouse.connect(alice).mint(quoteToken.address, toWei(10))
            expect(await vault.getFreeCollateral(alice.address)).to.eq(toWei(9, collateralDecimals))
        })
