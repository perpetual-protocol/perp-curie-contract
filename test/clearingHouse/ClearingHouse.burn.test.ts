...
describe("ClearingHouse.burn", () => {
    ...
    describe("burn quote when debt = 10", () => {
        beforeEach(async () => {
            ...
            expect(await vault.buyingPower(alice.address)).to.eq(toWei(10))

            // alice mints 10 quote
            await clearingHouse.connect(alice).mint(quoteToken.address, toWei(10))
            expect(await vault.buyingPower(alice.address)).to.eq(toWei(9))
        ...
        it("# can not burn more than debt, even there's enough available", async () => {
            ...
            // expect(await vault.buyingPower(alice.address)).to.eq(toWei(10).add(profit))
        })
    ...
    describe("burn base when debt = 10", () => {
        beforeEach(async () => {
            ...
            expect(await vault.buyingPower(alice.address)).to.eq(toWei(900))
        ...
        it("# burn base 10 when debt = 10, available = 10", async () => {
            ...
            expect(await vault.buyingPower(alice.address)).to.eq(toWei(1000))
        })
