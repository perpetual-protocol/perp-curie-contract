...
describe("ClearingHouse closePosition", () => {
    ...
    describe("one maker; initialized price = 151.373306858723226652", () => {
        ...
        it("one taker swaps base to quote and then closes", async () => {
            ...
            const [bobNetQuoteBalance, bobFee] = await accountBalance.getNetQuoteBalance(bob.address)
            ...
            const [aliceNetQuoteBalance, aliceFee] = await accountBalance.getNetQuoteBalance(alice.address)
            ...
            let feeMaker = (await accountBalance.getOwedAndUnrealizedPnl(alice.address))[2]
    ...
    describe("two makers; initialized price = 148.3760629", () => {
        ...
        it("ranges of makers are the same; alice receives 3/4 of fee, while carol receives only 1/4", async () => {
            ...
            const [aliceNetQuoteBalance, aliceFee] = await accountBalance.getNetQuoteBalance(alice.address)
            const [carolNetQuoteBalance, carolFee] = await accountBalance.getNetQuoteBalance(carol.address)
            ...
            let feeAlice = (await accountBalance.getOwedAndUnrealizedPnl(alice.address))[2]
            let owedOrUnrealizedPnlCarol = (await accountBalance.getOwedAndUnrealizedPnl(carol.address))[1]
            let feeCarol = (await accountBalance.getOwedAndUnrealizedPnl(carol.address))[2]
        ...
        describe("one maker closes position before the taker closes", () => {
            it("ranges of makers are the same", async () => {
                ...
                let owedRealizedPnlCarol = (await accountBalance.getOwedAndUnrealizedPnl(carol.address))[0]
                ...
                let [, , pendingFeeAlice] = await accountBalance.getOwedAndUnrealizedPnl(alice.address)
